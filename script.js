...
var nameslist = document.querySelector("#highlist");
...
function timer() {
    var timerInterval = setInterval(function () {
        ...
        if (seconds <= 0 || score === 3) {
            ...
            alert("Time's up!");
            highscore();
            var k = localStorage.getItem("Namess");
            for (i in k) {
                Names.textContent = k;
            }
...
function highscore() {
    ...
    submit.addEventListener("click", function () {
        var strings = (names.value + " With a score of: " + score + '\n ');
        storeNames.push(strings)
        localStorage.setItem("Namess", storeNames)
